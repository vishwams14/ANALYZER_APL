...
def login_page(league_config=None):
    """Render login page with league-specific branding"""
    ...
    if league_config:
        ...
        league_name = "Cricket Analytics"
        league_short = "CA"
        color_primary = "#667eea"
        color_secondary = "#764ba2"
        teams_list = []
    
    # Initialize auth manager with league-specific users file
    users_file = f'users_{league_short.lower()}.json' if league_config else 'users.json'
    ...
    with col2:
        ...
        st.markdown(f'<div class="login-title">🏏 {league_short} Analytics</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="login-subtitle">{league_name} 2025</div>', unsafe_allow_html=True)
            ...
            if submit:
                if not username or not password:
                    ...
                    if user_info:
                        ...
                        st.session_state.assigned_team = user_info['team']
...
def render_user_management(auth_manager, teams_list=None):
    ...
    with tab1:
        ...
        with st.form("add_user_form"):
            ...
            new_team = st.selectbox("Assign Team", teams_list)
